...
func (tiq *TodoItemQuery) QueryProject() *ProjectQuery {
	query := &ProjectQuery{config: tiq.config}
...
func (tiq *TodoItemQuery) Clone() *TodoItemQuery {
	...
	return &TodoItemQuery{
		...
		withProject: tiq.withProject.Clone(),
...
func (tiq *TodoItemQuery) WithProject(opts ...func(*ProjectQuery)) *TodoItemQuery {
	query := &ProjectQuery{config: tiq.config}
	for _, opt := range opts {
		opt(query)
	}
	tiq.withProject = query
	return tiq
}
...
func (tiq *TodoItemQuery) GroupBy(field string, fields ...string) *TodoItemGroupBy {
	grbuild := &TodoItemGroupBy{config: tiq.config}
...
func (tiq *TodoItemQuery) Select(fields ...string) *TodoItemSelect {
	...
	selbuild := &TodoItemSelect{TodoItemQuery: tiq}
	selbuild.label = todoitem.Label
...
func (tiq *TodoItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*TodoItem, error) {
	...
	if query := tiq.withProject; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*TodoItem)
		for i := range nodes {
			if nodes[i].project_items == nil {
				continue
			}
			fk := *nodes[i].project_items
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_items" returned %v`, n.ID)
			}
			for i := range nodes {
