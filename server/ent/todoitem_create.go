...
func (tic *TodoItemCreate) SetNillableUUID(u *uuid.UUID) *TodoItemCreate {
	if u != nil {
		tic.SetUUID(*u)
	}
	return tic
...
func (tic *TodoItemCreate) Save(ctx context.Context) (*TodoItem, error) {
	...
	if len(tic.hooks) == 0 {
		...
		nv, ok := v.(*TodoItem)
		if !ok {
			return nil, fmt.Errorf("unexpected node type %T returned from TodoItemMutation", v)
		}
		node = nv
	}
	return node, err
...
func (tic *TodoItemCreate) check() error {
	...
	if _, ok := tic.mutation.IsDone(); !ok {
		return &ValidationError{Name: "is_done", err: errors.New(`ent: missing required field "TodoItem.is_done"`)}
	}
	if _, ok := tic.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "TodoItem.created_at"`)}
	}
	if _, ok := tic.mutation.UUID(); !ok {
		return &ValidationError{Name: "uuid", err: errors.New(`ent: missing required field "TodoItem.uuid"`)}
	}
	if _, ok := tic.mutation.OwnerID(); !ok {
		return &ValidationError{Name: "owner_id", err: errors.New(`ent: missing required field "TodoItem.owner_id"`)}
	}
	return nil
}
