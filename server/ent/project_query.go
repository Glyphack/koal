...
func (pq *ProjectQuery) QueryItems() *TodoItemQuery {
	query := &TodoItemQuery{config: pq.config}
...
func (pq *ProjectQuery) WithItems(opts ...func(*TodoItemQuery)) *ProjectQuery {
	query := &TodoItemQuery{config: pq.config}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context) ([]*Project, error) {
	...
	if query := pq.withItems; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Items = []*TodoItem{}
		}
		query.withFKs = true
		query.Where(predicate.TodoItem(func(s *sql.Selector) {
			s.Where(sql.InValues(project.ItemsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.project_items
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "project_items" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_items" returned %v for node %v`, *fk, n.ID)
			}
